  ...
  sidebarLayout(
    sidebarPanel(
      ...
      checkboxGroupInput(inputId = 'source',
                         label="Category",
                         choices = sort(unique(all_facilities_clean$source)),
                         selected = unique(all_facilities_clean$source)),
...
server <- function(input, output, session) {
  ...
  #Using all_facilities_clean data, this data is generated by final_project_data_processing.qmd code
  #final_project_data_processing.qmd code is also submitted
  all_facilities_clean <- read.csv("all_facilities_clean.csv", row.names = NULL)
  facility_colors <- c(
    # recreation
    "public park"   = "#1b9e77",
    "golf course"   = "#66a61e",
  ...
  filtered <- reactive({
    ...
    all_facilities_clean%>% filter(source %in% input$source)%>%
  ...
  output$map <- renderLeaflet({
    ...
    leaflet(points_of_int) %>%
      ...
      addCircleMarkers(lng=~lon, lat=~lat, radius=5, stroke=FALSE, fillOpacity = 1,
                       color = ~pal(facility_type),
                       popup=~paste0("<b>", name, "</b><br>",
                                     facility_type, "<br>",
                                     "Data Source: ", source))%>%
      addLegend(position = "bottomright", pal = pal, values = ~facility_type, title = "Facility Type", opacity = 1)
  }
  )
}
